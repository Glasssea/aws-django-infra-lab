...
def get_summonerInfo(summonerName): # summonerName -> summonerId /SOMMONER-V4
    url = f"https://kr.api.riotgames.com/lol/summoner/v4/summoners/by-name/{summonerName}" #옛날거
    headers = {
...
# print(leadue_v4__encryptedsummonerid("N81vdRYa0gDBdbP-lHQ2nvjKPJyAz8lqKAKN_snP1sBi7A"))
...
def get_mastery_bypuuid(puuid):
    ...
    return final_df
# df = get_mastery_bypuuid('soerso-LMacN5eiMW2dqWEHA7br3adVNQgoDnotBXcOTFLBFBytjDTRO6JZAzhebWO8zlCebUGda-w')
# print(df)

###############
###############
###############
############### 지금은 get_mastery 사용하고 있는데, 위에 puuid로 바꾸면 더 편할듯?
###############
###############
###############
def get_mastery(summonerId):#소환사 아이디로 마스터리 검색하고 검색해서 나온 숙련도 정보를 puuid 와 함께 csv 파일로 덮어써서 저장
    ...
    return final_df
# df = get_mastery('N81vdRYa0gDBdbP-lHQ2nvjKPJyAz8lqKAKN_snP1sBi7A')
# print(df)
...
input_nickname = 'troll bat'
# point_ML(get_mastery(get_summonerInfo(input_nickname).get('id')))
...
# info = get_summonerInfo('troll bat')
...
# data = {'info':info, 'solo':solo, 'free':free, 'winrate':winrate_rated, 'tagLine':tagLine, 'recommend_champ1':champion_names[0], 'recommend_champ2':champion_names[1], 'recommend_champ3':champion_names[2]}
# print(data)
# 
...
puuid = account_v1__gamename_tagline("trollbat","kr1")['puuid']
id = summoner_v4__encryptedpuuid(puuid)['id']
print(leadue_v4__encryptedsummonerid(id)[0])
get_mastery_bypuuid(puuid)